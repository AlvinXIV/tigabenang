...
<div class="space-y-6" x-data="{ autoRotate: true }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-[#E2E5E9]">
        <div>
            ...
            <div class="flex items-center gap-3">
                <h1 class="text-2xl sm:text-3xl font-bold text-[#102A43] tracking-tight">{{ $product->nama_produk }}</h1>
                <span class="px-2.5 py-0.5 text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-md">
                    3D Model Aktif
                </span>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="lg:col-span-2 admin-card p-4 space-y-3">
            <div class="h-[460px] w-full bg-[#F7F7F5] border border-[#E2E5E9] rounded-xl relative overflow-hidden shadow-inner">
                <model-viewer
                    ...
                    exposure="1.1"
                    class="w-full h-full cursor-grab active:cursor-grabbing bg-[#F7F7F5]"
                ></model-viewer>

                <div class="absolute top-3.5 left-3.5">
                    <span class="px-3 py-1.5 bg-white/95 backdrop-blur-xs text-[#102A43] text-xs font-semibold border border-[#E2E5E9] rounded-lg shadow-[0_1px_2px_rgba(16,42,67,0.04)] flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                        Pratinjau Interaktif 3D
                    </span>
                </div>

                <div class="absolute bottom-3.5 right-3.5 bg-white/95 backdrop-blur-xs px-3 py-1.5 border border-[#E2E5E9] rounded-lg shadow-[0_1px_2px_rgba(16,42,67,0.04)] flex items-center gap-2 text-xs text-[#102A43]">
            ...
            <div class="px-1 text-xs text-[#667085] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1">
                ...
                <span class="font-mono text-[11px] text-[#102A43] font-semibold">📁 {{ basename($product->file_model_3d) }}</span>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="admin-card p-5 space-y-4">
                <h2 class="text-sm sm:text-base font-semibold text-[#102A43] border-b border-[#E2E5E9] pb-3">Spesifikasi Model</h2>

                <div class="space-y-3 text-xs sm:text-sm divide-y divide-[#E2E5E9]">
                    <div class="flex justify-between py-2">
                        <span class="text-[#667085]">Nama File:</span>
                        <span class="font-mono font-semibold text-[#102A43] truncate max-w-[150px]">{{ basename($product->file_model_3d) }}</span>
                    </div>
                    <div class="flex justify-between py-2">
                    ...
                    <div class="flex justify-between py-2">
                        ...
                        <span class="font-semibold text-[#102A43]">Rp {{ number_format($product->harga, 0, ',', '.') }}</span>
                    </div>
                </div>
